...
from biotrainer_core.data_classes.autoeval import AutoEvalReport, SupervisedFrameworkReport, ZeroShotFrameworkReport, \
    ContactFrameworkReport, UnsupervisedFrameworkReport
...
def _render_unsupervised(autoeval_report: AutoEvalReport, dev_mode: bool):
    fw_names = list(autoeval_report.unsupervised_results.keys())
    fw_sel = st.selectbox("Framework", options=fw_names)
    unsup_rep: UnsupervisedFrameworkReport = autoeval_report.unsupervised_results[fw_sel]
    tasks = unsup_rep.get_task_names()
    if not tasks:
        st.info("No tasks available.")
        return
    task = st.selectbox("Task", options=tasks)
    df_unsup = unsup_rep.to_df(all_metrics=True, development_mode=dev_mode)
    df_task = df_unsup[df_unsup["Task"] == task] if not df_unsup.empty else df_unsup
    df_task = _postprocess_task_df(df_task)
    st.dataframe(df_task, use_container_width=True, hide_index=True)
    _build_metrics_chart(df_task)


def _render_zeroshot(autoeval_report: AutoEvalReport, dev_mode: bool):
...
def _render_contact(contact_results: Dict[str, ContactFrameworkReport], kind: str, dev_mode: bool):
    ...
    for tab, (_, kind) in zip(tabs, framework_tab_names):
        with tab:
            if kind == "supervised":
                ...
                _render_unsupervised(autoeval_report, dev_mode)
            elif kind == "zeroshot":
                _render_zeroshot(autoeval_report, dev_mode)
                ...
                _render_contact(contact_results, kind, dev_mode)
