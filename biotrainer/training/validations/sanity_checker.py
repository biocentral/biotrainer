...
class SanityChecker:
        ...
        if self.protocol in Protocol.classification_protocols():
            ...
            if self.n_classes > 2:
                ...
                if accuracy == precision == recall:
                    self._handle_sanity_check_warning(
    ...
    def _random_model_initialization_baseline(self):
        """
        Uses a model with randomly initialized weights and no training to predict the test set.

        If the actual model did not train, the model would be similar to this baseline.
        """
    ...
    def _random_sampling_baseline(self):
        """
        Uses the distributions of classes or values in the train set to predict the test set.
        """
        ...
        random_sampling_baseline = Bootstrapper.bootstrap(
    ...
    def _bias_interaction_baseline(self):
        """
            ...
            the higher value.
        """
        assert (self.protocol in Protocol.per_sequence_protocols()
                and self.protocol in Protocol.classification_protocols())
